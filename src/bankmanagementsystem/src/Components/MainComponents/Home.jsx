                    ...
                    <div className="card-body">
                        <div className="container row mt-3">
                            <div className="col-6" style={{ "textAlign": "left" }}>
                                <Button variant="contained" data-testid='addPayee' name="addPayee" onClick={() => { setFlags({ isAddable: true }) }} color="success">Add Payee</Button></div>
                            <div className="col-6">
                                ...
                                <tbody>
                                    {
                                        data.filter((st) => st.payeeName.toLowerCase().includes(search) || st.payeeName.toUpperCase().includes(Search)).map((item) => (
                                            <tr key={item.payeeId}>
                                                ...
                                                <td className="d-flex justify-content-evenly"><Button variant="contained" name={"edit"+(editCount++)} onClick={() => { setFlags({isEditable:true});setEventId(item.payeeId)}}>edit</Button><Button variant="contained" color="error" name={"delete"+(deleteCount++)} onClick={() => { setFlags({isDeletable:true});setEventId(item.payeeId) }}>delete</Button></td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
